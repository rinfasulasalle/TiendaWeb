...
export default {
  ...
  methods: {
    ...
    agregarTipoUsuario() {
      axios
        .put("http://localhost:5000/usuario", this.nuevoTipoUsuario)
        .then((response) => {
    ...
    enviarEdicion() {
      ...
      axios
        .patch("http://localhost:5000/usuario", dataToUpdate)
        .then((response) => {
          // Handle the response (if needed)
          // For example, you can display a success message and refresh the data table
    ...
    eliminarRegistro() {
      ...
      axios
        .delete(`http://localhost:5000/usuario`, {
